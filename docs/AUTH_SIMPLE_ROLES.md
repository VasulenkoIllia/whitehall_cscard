...
- Поле логіну приймає email або короткий логін (локальна частина). За замовчуванням дозволено `admin@example.com` **або** просто `admin` з паролем `admin` (див. `AUTH_USERS_JSON` в `.env`).
