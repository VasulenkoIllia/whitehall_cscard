...
- `DEPLOYMENT.md` — середовища, сервер, Traefik/Cloudflare, `.env` і compose, sparse-checkout,
  workflow розробки, деплой TEST/PROD, паузи перед міграціями з бекфілом, відкат.
- `RUNBOOK_SERVER_MIGRATION_2026_08.md` — перенос PROD і TEST на новий сервер у ніч на 15.08.2026:
  порядок дій, методика звірки даних, виміряні тривалості, ухвалені рішення, пастки, відкладені борги
  (ротація ключів, бекапи).
