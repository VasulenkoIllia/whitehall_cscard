...
/**
 * ExcelImportModal — імпорт товарів з Excel у master_catalog (feed_params для AI).
 *
 * Флоу: вибір файлу → preview (аркуші, заголовки, перші очищені рядки, підказки
 * SKU/виключених колонок) → користувач коригує → імпорт → підсумок. Файл тримається
 * у state браузера і відправляється двічі (preview + import) — сервер нічого не
 * зберігає на диску. Усі колонки крім SKU/фото лягають у feed_params.excel_upload,
 * звідки їх читає AI-enrichment.
 */
export function ExcelImportModal({ apiFetch, onClose, onAfterImport }) {
  ...
  const runImport = async () => {
    ...
    if (!window.confirm(
      ...
      `Існуючі SKU оновляться, нові — створяться. Дані підуть у feed_params для AI.`
    )) return;
    setImporting(true);
    setError('');
    try {
        ...
        {preview && !importResult ? (
          <>
            ...
            <details open style={{ marginBottom: 12 }}>
              <summary style={{ cursor: 'pointer', fontWeight: 600, color: '#4a90e2' }}>
                👁 Перші рядки після очистки (що піде в AI)
              </summary>
